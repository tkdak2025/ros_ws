...
가상 Recipe: `RECIPE_A` = PASS / FAIL(변위 초과) / MISSING(미끄러짐) / INCOMPLETE(시간 초과),
`RECIPE_B` = PASS / FAIL(최대 거리) / PASS / PASS.
제품 결과는 `PASS / FAIL / MISSING` 3종이고, 판정을 만들지 못한 Point 는 `INCOMPLETE` 로 따로 둔다.
상세 원인은 `reason_code`, 판정 진행은 `judgment_status` 다 (통합문서 Sequence #06, 2026-09-22).
...
- **테이블 구조는 아직 정하지 않았다.** `recipe_db.py` 는 실제 테이블을 모르고 약속된 뷰 `v_recipe_point` 하나만 조회한다. 테이블을 설계한 뒤 아래 컬럼 이름으로 묶는 뷰만 만들면 코드 수정 없이 값이 채워진다.
  ...
  | `max_displacement_mm`, `required_pull_force_n`, `pull_force_limit_n`, `repeat_count`, `grip_width_mm` | 선택 |

  `required_pull_force_n` 은 **합격선이면서 Pull 정지 조건**이다 (LAN 20 N / USB 12 N). 허용 변위
  `max_displacement_mm` 5 mm 는 판정 기준일 뿐 정지 조건이 아니다. PASS = 기준 힘 도달 AND 변위 ≤ 5 mm.
  `pull_force_limit_n` 은 기준 힘과 정지 힘을 따로 두던 때의 옛 필드다(값이 다를 때만 화면에 나온다).
...
실행 전에 `sod` 와 `source ~/ros_ws/install/setup.bash` 둘 다 필요하다. 실험용 예제 코드(`inspect_async.py`, `sequence_demo.py`)는 `hmi_test` 브랜치에 있다.
...
- 두산 파이썬 라이브러리에는 `move_pause` / `move_resume` 함수가 없다. 서비스 `dsr_controller2/motion/move_pause`,
  `.../move_resume` (`dsr_msgs2/srv/MovePause`, `MoveResume`) 를 직접 부른다.
...
- **FAIL / MISSING 포인트 이동**: `wait_for_start()` 대신 `name, args = progress.wait_for_command()` 를 쓰면 대기 중에 '검사 시작' 뿐 아니라
  ...
  검사 중과 똑같이 받고 '검사 시작' 은 잠긴다), 끝나고 `progress.moved()`(이동 전 상태로 복귀, 결과 표·제품 판정은 그대로).
  티칭 안 된 Point 와 레시피에 없는 Point 는 거부하고 시스템 로그에 남기는 것을 권한다.
...
- 이 절의 기능을 전부 쓴 시험 코드(`SYSTEM_READY → START VALIDATION → Work Initialize → Home Return →
  [Point Unit] → Work Finish → Home Return` 순서, Safe Pause Point 방식의 일시정지, 통신 단절 감시, Home Return)는
  실험용 예제 코드(`inspect_async.py`, `sequence_demo.py`)는 `hmi_test` 브랜치에 있다.
...
- 검사 뼈대 예제와 가상 시험용 레시피 생성 스크립트는 실험용 예제 코드(`inspect_async.py`, `sequence_demo.py`)는 `hmi_test` 브랜치에 있다.
