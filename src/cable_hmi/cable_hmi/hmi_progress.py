...
3) 검사 결과까지 보고할 때 ('현재 검사 결과' 표, 상세 팝업, 제품 판정):
    ...
    progress.point(i, point_id, criteria=itf.Criteria(...))   # 판정 기준 칸에 표시된다
    ...
...
class _CommandListener:
    ...
    def __init__(self, context, topic, on_command, on_tick):
...
class ProgressReporter:
    ...
    def point(self, index: int, name: str = '', criteria: itf.Criteria = None):
        """Point 하나를 시작한다. index 는 0 부터 센다. criteria 를 주면 HMI 판정 기준 칸에 표시된다."""
        with self._lock:
    ...
    def wait_for_command(self):
        ...
        START 면 0 % 로 시작을 알린 뒤에 돌아온다(args 예: {'recipe_id': ...}).
        MOVE_TO_POINT 면 상태를 바꾸지 않고 돌아온다(args: {'point_id': ..., 'reason': 'FAIL'|'MISSING'}).
        이동하기 전에 moving(), 끝나고 moved() 를 부를 것. control=False 면 기다리지 않고 START 를 돌려준다.
        """
    ...
    def moving(self, point_id: str):
        """포인트 이동을 시작한다고 알린다 (HMI '이동 중'). 진행률·결과·제품 판정 표시는 그대로 둔다."""
        with self._lock:
            self._before_move = (self._state.run_state, self._state.step)
    ...
    def moved(self):
        ...
        with self._lock:
            if self._before_move is None:
                return
            self._state.run_state, self._state.step = self._before_move
            self._before_move = None
            self._publish()
    ...
    def check_pause(self, pause=None, resume=None) -> bool:
        ...
        if pause is not None and not pause():
            ...
            return False
        resumed_state = self._state.run_state       # RUNNING 또는 MOVING
        self._set_run_state(itf.State.PAUSED)
        while True:
    ...
    def _on_command(self, msg):
        ...
        if cmd.name == itf.CommandName.ESTOP:
            ...
            if run_state in (itf.State.IDLE, itf.State.DONE) and not self._start.is_set():
                self._pending = (cmd.name, dict(cmd.args))
                self._start.set()
        elif cmd.name == itf.CommandName.PAUSE:
            if run_state in (itf.State.RUNNING, itf.State.MOVING):
                self._pause.set()
        elif cmd.name == itf.CommandName.RESUME:
            if run_state == itf.State.PAUSED:
                self._resume.set()
    ...
    def _set_run_state(self, run_state: str):
        with self._lock:
