...
시험 좌표는 `src/recipe/error_validation/`에 축별 레시피로 보관한다.
결과는 기존과 동일하게 프로젝트의 `measurement_results/`에 저장한다.

## 레시피 기반 Y축 시험

```bash
python3 src/error_validation/pose_range_test.py \
...
```bash
python3 src/error_validation/pose_range_test.py \
  --recipe src/recipe/error_validation/z_pose_range_recipe.json
```

## 수동 위치 관찰

```bash
python3 src/error_validation/pose_range_observer.py
```
